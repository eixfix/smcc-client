...
export async function POST(
  ...
  const response = await fetch(`${apiBaseUrl}/servers/${serverId}/agents`, {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    ...
    return NextResponse.json({ error: errorPayload.message ?? 'Failed to create agent.' }, { status: response.status });
  }
